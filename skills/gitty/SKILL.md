...
2. Run `gitty providers` if provider availability is unknown. If `gitty` is missing, tell the user and offer the installation command from [CLI reference](references/cli.md); do not silently install software.
...
- Use `--dry-run` when the user wants to review the exact provider prompt or when sensitive diff contents may be sent to an AI provider.
